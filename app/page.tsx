...
const brands = [
  ...
  ["Frimesa", "/brands/frimesa-color.png"],
  ...
  return (
    <main id="inicio">
      ...
      <section className="hero" id="conteudo">
        <motion.div className="hero-copy" initial={{ opacity: 0, x: reduceMotion ? 0 : -36 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: reduceMotion ? 0 : 0.72 }}><h1>Gestão inteligente da granja <em>Do produtor independente à integradora</em></h1><p className="hero-lead">Controle lotes, mortalidade, ração, custos e resultados em uma plataforma desenvolvida para a realidade da suinocultura</p><div className="hero-actions"><a className="button" href="#gestao">Conhecer GranjaFlow Gestão <Arrow /></a><a className="button-secondary" href="#integra">Conhecer GranjaFlow Integra <Arrow /></a></div><div className="trust-row"><span><CheckIcon /></span><p><strong>Comece pela sua realidade</strong><br />Gestão para o produtor e Integra para operações integradas</p></div></motion.div>
      ...
      <section className="solution-choice" aria-labelledby="choice-title"><Reveal className="choice-heading"><h2 id="choice-title">Um GranjaFlow para cada modelo de produção</h2><p>Escolha o caminho que representa sua operação hoje</p></Reveal><div className="solution-grid"><Reveal className="solution-card solution-primary" side="left"><div className="solution-card-head"><span>PARA PRODUTOR INDEPENDENTE</span><GrainIcon type="money" /></div><h3>GranjaFlow Gestão</h3><p>Para quem administra a própria produção e precisa enxergar o que está acontecendo operacionalmente e economicamente em cada lote</p><ul>{managementBenefits.map(item => <li key={item}><CheckIcon />{item}</li>)}</ul><a className="button" href={gestaoWhatsapp} target="_blank" rel="noreferrer">Quero controlar minha granja <Arrow /></a></Reveal><Reveal className="solution-card solution-secondary" side="right"><div className="solution-card-head"><span>PARA INTEGRADORAS E COOPERATIVAS</span><GrainIcon type="network" /></div><h3>GranjaFlow Integra</h3><p>Para operações que precisam acompanhar técnicos, produtores, granjas e lotes dentro de uma estrutura integrada</p><ul>{integraBenefits.map(item => <li key={item}><CheckIcon />{item}</li>)}</ul><a className="button button-navy" href={integraWhatsapp} target="_blank" rel="noreferrer">Conhecer solução para integradoras <Arrow /></a></Reveal></div></section>

      <section className="producer-section" id="gestao"><Reveal className="producer-copy" side="left"><h2>Sua granja, seus números, suas decisões</h2><p>Gastos separados não mostram sozinhos o custo real do lote, o GranjaFlow Gestão reúne produção, consumo e financeiro para transformar os registros do dia a dia em uma visão clara</p><div className="producer-question"><strong>Quanto cada suíno está me custando hoje?</strong></div><a className="button button-light" href={gestaoWhatsapp} target="_blank" rel="noreferrer">Solicitar demonstração do Gestão <Arrow /></a></Reveal><Reveal className="producer-dashboard" side="right"><DemoMetrics /></Reveal></section>
      ...
      <section className="ecosystem-section" aria-labelledby="ecosystem-title"><Reveal className="ecosystem-heading"><h2 id="ecosystem-title">Referências da suinocultura Brasileira</h2></Reveal><div className="brand-carousel" aria-label="Empresas do setor agroindustrial Brasileiro"><div className="brand-track">{[...brands, ...brands].map(([name, src], index) => <div className="industry-brand" key={`${name}-${index}`}><img src={src} alt={name} /></div>)}</div></div></section>
